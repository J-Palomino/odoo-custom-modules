...
One Odoo `mint.discount` record can target N stores (m2m `store_ids`).
Dutchie's `update-discount-item` endpoint is keyed by `LocId`, so each
(discount, store) pair becomes one Dutchie discount record. This link
table tracks the per-store sync state and the Dutchie ID returned on
create.

Complements `mint.dutchie.discount.push.log` (which records per-attempt
audit history): this table answers "what's the current sync state of this
discount at this store?" while the audit log answers "what happened on
each push attempt?". Both are populated by the same publish path.
"""
...
class MintDiscountDutchieLink(models.Model):
    ...
    # Stored as Char to match the type of mint.discount.dutchie_discount_id
    # (also Char, defined in mint_api_v2).
    ...
    def create(self, vals_list):
        ...
        for vals in vals_list:
            if vals.get('dutchie_loc_id') in (None, 0) or vals.get('dutchie_lsp_id') in (None, 0):
                comp = self.env['res.company'].browse(vals.get('location_id'))
                if comp.exists():
                    vals.setdefault('dutchie_loc_id', comp.dutchie_loc_id or 0)
                    vals.setdefault('dutchie_lsp_id', comp.dutchie_lsp_id or 0)
