...
CALC_BY_TYPE = {
    ...
    # a percent value just like 'percent' (see _format_sales_details, which
    # renders "N% Off (Clearance)"). Without this entry _dutchie_build raised
    # "Unsupported discount type 'clearance'", which the convert try/except
    # swallowed, so clearance deals silently never reached Dutchie.
    'clearance': 2,
}
...
class DealSubmissionDutchiePublish(models.Model):
    ...
    def action_convert_to_deal(self):
        ...
        # NOTE: clearance is NOT exempt — it now maps to PERCENT_OFF and needs
        # its percent, so a value-less clearance is caught here with a clear
        # "set the value" message instead of publishing a 0%-off discount.
