...
async function callDaisy(question) {
    const res = await fetch(DAISY_API_URL, {
        method: "POST",
        ...
        try {
            ...
            setTimeout(async () => {
                try {
                    ...
                    try {
                        const answer = await callDaisy(question);

                        const container = body.querySelector(".daisy-error-replaced");
                        container.innerHTML =
                            // AI response (primary content)
                        ...
                        // API failed — show original error as fallback
